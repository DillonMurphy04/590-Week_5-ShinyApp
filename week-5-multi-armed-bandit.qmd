...
$$
p_k \mid \text{data} \sim \operatorname{Beta}(\alpha_k,\beta_k),
\qquad
\alpha_k \leftarrow \alpha_k + \text{successes},\;
\beta_k \leftarrow \beta_k + \text{failures}.
$$
Each round:
...
1. In **Play (Multi-Arm)**, set $K=4$, **Randomize arms**, Horizon $=100$.
2. Run **$\varepsilon$-Greedy** with $\varepsilon=0.30$ and then with $\varepsilon=0.05$. Compare **Cumulative Regret** and **Pulls by Arm**.
3. On the same instance (click **Reset** once to fix a new draw), run **UCB1** and **TS**.

**Questions.**
- Which policy locks onto the best arm fastest? What do you see in **Pulls by Arm**?
- How does increasing $\varepsilon$ change early vs. late performance?
- Does TS allocate extra pulls to uncertain arms early on? Why would that help?
...
For a fixed horizon $H$ and arm configuration $\{p_k\}$, it runs each policy multiple times and records **terminal regret** $\mathcal{R}(H)$. It summarizes **mean regret** and **95% intervals** (empirical quantiles).
...
2. Inspect the **mean $\pm$ 95% CI** and the raw table.
3. Turn off randomization and set arms close together (e.g., $p = [0.56, 0.54, 0.53, 0.50]$). Re-run.

**Questions.**
- Which policy has the **lowest mean terminal regret**? Are differences practically meaningful given the CIs?
- How do the CIs change when arms are harder to distinguish (smaller gaps)?
...
# 5. Interpreting Every App Element

- **“Pulls per click” (Learn tab):** how many iid Bernoulli draws you add before re-plotting the posterior.
- **“Success” vs. “Failure”:** realized reward $1$ vs. $0$ on a pull.
- **“Reveal true $p$” (Learn tab):** shows the (hidden) data-generating probability so you can check calibration.
- **“Horizon” (Play/Compare):** total number of decisions (rounds).
- **“Cumulative Regret”:** $\mathcal{R}(t)$; lower is better; slope reflects learning speed.
- **“Pulls by Arm”:** counts $n_k(t)$; the best arm’s bar should dominate as $t$ grows.
- **“History table”:** per-round log (arm chosen, reward, cumulative reward, oracle best-possible reward $t \cdot p_*$, and regret).

**Questions.**
- If **Pulls by Arm** shows heavy sampling of an obviously weak arm late in the run, what might be wrong with your policy/parameters?
- If **Cumulative Regret** bends downward sharply mid-run, what event likely occurred?
